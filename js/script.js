/**
 * ====================================================================
 * PF SOLIDAIRE ERP - LOGIC V11 (STOCKS + COMPLET)
 * ====================================================================
 */
...
window.showSection = function(id) {
    ...
    document.getElementById('view-stock').classList.add('hidden'); // NEW
    document.getElementById('view-' + id).classList.remove('hidden');
    if(id === 'base') window.chargerBaseClients();
    if(id === 'stock') window.chargerStock(); // NEW
};
...
window.toggleSections = function() {
    ...
    if(type === 'Inhumation') { document.getElementById('bloc_inhumation').classList.remove('hidden'); document.getElementById('btn_inhumation').classList.remove('hidden'); } 
    else if(type === 'Crémation') { document.getElementById('bloc_cremation').classList.remove('hidden'); document.getElementById('btn_cremation').classList.remove('hidden'); } 
    else if(type === 'Rapatriement') { document.getElementById('bloc_rapatriement').classList.remove('hidden'); document.getElementById('btn_rapatriement').classList.remove('hidden'); }
};
...
window.ajouterArticleStock = async function() {
    ...
    try {
        await addDoc(collection(db, "articles_stock"), {
            nom, cat, qte, pa, pv, fourn, date_ajout: new Date().toISOString()
        });
        alert("✅ Article ajouté !");
        document.getElementById('form-stock').classList.add('hidden');
        document.getElementById('st_nom').value = ""; // Reset
        window.chargerStock();
    } catch(e) { alert("Erreur : " + e.message); }
};
...
window.chargerStock = async function() {
    ...
    try {
        const q = query(collection(db, "articles_stock"), orderBy("nom"));
        ...
        snap.forEach(docSnap => {
            ...
            const badgeClass = d.qte < 3 ? "stock-alert" : "stock-ok";
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td><strong>${d.nom}</strong><br><small style="color:#64748b;">${d.fourn || ''}</small></td>
                <td><span class="badge">${d.cat}</span></td>
                <td>${d.pa} €</td>
                <td>${d.pv} €</td>
                <td><span class="badge ${badgeClass}" style="font-size:1rem;">${d.qte}</span></td>
                <td style="text-align:center;">
                    <button class="btn-icon" onclick="window.modifStock('${docSnap.id}', ${d.qte}, 1)" title="+1"><i class="fas fa-plus" style="color:green;"></i></button>
                    <button class="btn-icon" onclick="window.modifStock('${docSnap.id}', ${d.qte}, -1)" title="-1"><i class="fas fa-minus" style="color:orange;"></i></button>
                    <button class="btn-icon" onclick="window.supprimerArticle('${docSnap.id}')" title="Supprimer"><i class="fas fa-trash" style="color:red;"></i></button>
                </td>
            `;
...
window.modifStock = async function(id, currentQte, delta) {
    const newQte = currentQte + delta;
    if(newQte < 0) return alert("Stock ne peut pas être négatif.");
    try {
        await updateDoc(doc(db, "articles_stock", id), { qte: newQte });
        window.chargerStock();
    } catch(e) { alert("Erreur : " + e.message); }
};
...
window.supprimerArticle = async function(id) {
    if(confirm("Supprimer cet article définitivement ?")) {
        try { await deleteDoc(doc(db, "articles_stock", id)); window.chargerStock(); }
        catch(e) { alert("Erreur : " + e.message); }
    }
};
...
async function sauvegarderEnBase() {
    ...
    try {
        ...
        if (dossierId) { await updateDoc(doc(db, "dossiers_admin", dossierId), dossierData); alert("✅ Dossier mis à jour !"); } 
        else { dossierData.date_creation = new Date().toISOString(); await addDoc(collection(db, "dossiers_admin"), dossierData); alert("✅ Dossier créé !"); }
...
window.chargerDossier = async function(id) {
    try {
        ...
        if (docSnap.exists()) {
            ...
            window.showSection('admin'); document.getElementById('dossier_id').value = id;
...
window.goToFacturation = function(nomDefunt) {
    if(nomDefunt) { window.location.href = `facturation_v2.html?search=${encodeURIComponent(nomDefunt)}`; } else { window.location.href = `facturation_v2.html`; }
};
...
function headerPF(pdf, y=20) {
    pdf.setFont("helvetica","bold"); pdf.setTextColor(34,155,76); pdf.setFontSize(12); pdf.text("POMPES FUNEBRES SOLIDAIRE PERPIGNAN",105,y,{align:"center"});
    pdf.setTextColor(80); pdf.setFontSize(8); pdf.setFont("helvetica","normal"); pdf.text("32 boulevard Léon Jean Grégory Thuir - TEL : 07.55.18.27.77",105,y+5,{align:"center"}); pdf.text("HABILITATION N° : 23-66-0205 | SIRET : 53927029800042",105,y+9,{align:"center"});
...
window.genererDemandeRapatriement = function() {
    ...
    if(getVal("vol1_num")) { 
        pdf.setFont("helvetica", "bold"); pdf.text(`- Vol 1 : ${getVal("vol1_num")} (${getVal("vol1_dep_aero")} -> ${getVal("vol1_arr_aero")})`, x+20, y); y+=5;
        pdf.setFont("helvetica", "normal"); pdf.text(`  Départ : ${getVal("vol1_dep_time")}`, x+25, y); pdf.text(`  Arrivée : ${getVal("vol1_arr_time")}`, x+90, y); y+=7;
    }
    if(document.getElementById('check_vol2').checked && getVal("vol2_num")) { 
        pdf.setFont("helvetica", "bold"); pdf.text(`- Vol 2 : ${getVal("vol2_num")} (${getVal("vol2_dep_aero")} -> ${getVal("vol2_arr_aero")})`, x+20, y); y+=5;
        pdf.setFont("helvetica", "normal"); pdf.text(`  Départ : ${getVal("vol2_dep_time")}`, x+25, y); pdf.text(`  Arrivée : ${getVal("vol2_arr_time")}`, x+90, y); y+=7;
    }
    y+=5;
...
window.genererDeclaration = function() {
    ...
    if(prof === "Active") { const metier = getVal("profession_libelle") || "Active"; pdf.setFont(fontMain, "bold"); pdf.text(metier.toUpperCase(), margin+110, y); }
    y += 15;
...
window.genererDemandeInhumation = function() {
    ...
    pdf.setFont("helvetica", "normal"); pdf.text("Veuillez agréer, Monsieur le Maire, mes sal
