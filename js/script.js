/* Fichier : js/script.js - VERSION CORRIGÉE (RAPATRIEMENT EXPERT RESTAURÉ) */
...
window.chargerBaseClients = async function() {
    ...
    try {
        ...
        snap.forEach(docSnap => {
            ...
            tr.innerHTML = `<td>${dateC}</td><td><strong>${defunt}</strong></td><td>${data.mandant?.nom || '-'}</td><td><span class="badge">${op}</span></td>
                <td style="text-align:center;"><button class="btn-icon" onclick="window.showSection('admin')"><i class="fas fa-edit"></i></button></td>`;
...
// ==========================================================================
// 4. FONCTIONS PDF - RESTAURATION COMPLÈTE
// ==========================================================================

// --- RAPATRIEMENT (VOTRE VERSION EXPERTE - RESTAURÉE) ---
